use axum::{Json, http::StatusCode, response::IntoResponse, http::header, body::Body};
...
pub async fn download_code(Json(payload): Json<DownloadCode>) -> impl IntoResponse {
  ...
  match common::download::code(&path).await {
    Ok(file_bytes) => {
      ...
      let filename = format!("{}-{}.{:?}", payload.chat_id, payload.message_id, path.extension());
    ...
    Err(err) => {
      ...
      (StatusCode::INTERNAL_SERVER_ERROR, format!("下载代码失败: {}", err)).into_response()
    }
  }
}
