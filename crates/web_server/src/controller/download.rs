use crate::service::{dashboard, metadata_info};
...
async fn get_dashboard_config(
  ...
  let dashboard_json = serde_json::to_string(&dashboard_config)
    .map_err(|e| AppError::DashboardSerializationError(e.to_string()))?;
...
async fn get_metadata_options(db: &DbConn) -> Result<Vec<DownloadCodeOption>, AppError> {
  ...
  let metadata_all = metadata_info::find_all(db)
    .await
    .map_err(|e| AppError::MetadataQueryError(e.to_string()))?;
