...
export const analyzeDailySummary = sqliteTable("analyze_daily_summary", {
	id: text().primaryKey().notNull(),
...
export const analyzeOrderProductDetails = sqliteTable("analyze_order_product_details", {
	id: text().primaryKey().notNull(),
	customerCode: integer("customer_code").notNull(),
...
(table) => [
	primaryKey({ columns: [table.chatId, table.messageId], name: "dashboard_chat_id_message_id_pk"})
]);
